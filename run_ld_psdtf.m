function aa = run_ld_psdtf(name, iterations)
% name = '01', '02'...
%iterations = 200
pkg load signal;
method_folder = 'ld_psdtf';
M = 512;
N = 1440;
K = 3;
timestart = clock;
...
[x, s, fs, bits] = ld_psdtf_bss(strcat('./new_samples/16000/', name ,'_mix.wav'), M, N, K, iterations);
for k = 1 : K
  wavwrite(s(:, k), fs, bits, ['./sources/ld_psdtf/' name '_' ['1', '2', '3'](k) '.wav']);
end
