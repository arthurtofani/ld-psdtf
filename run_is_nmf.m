function aa = run_is_nmf(name, iterations)
% name = '01', '02'...
%iterations = 200
pkg load signal;
method_folder = 'is_nmf';
M = 512;
N = 1440;
K = 3;
timestart = clock;
...
[x, s, fs, bits] = is_nmf_bss(strcat('./new_samples/16000/', name ,'_mix.wav'), M, N, K, iterations);
for k = 1 : K
  wavwrite(s(:, k), fs, bits, ['./sources/is_nmf/' name '_' ['1', '2', '3'](k) '.wav']);
end
